%%10.17：iir作为尾部拼接。相当于并行逻辑。但是缓冲区的初始化80ms还需要确定。因为我忘记了lars怎么设置的四小段的长度。接近80ms但有误差，不能保证是否出现clack
%这个文件也没有得到验证，不知道能不能运行。
addpath('utils');
SetParameters;
...
while true % endless
  ...
  if ~bHeadphone
    ...
    for iCRx = 1:2
      for iCTx = 1:iNoTx
        % IIR 处理，输入是延迟后的信号，并更新状态
        [mOut_IIR_temp, mIIRReg(:,iCRx,iCTx)] = ...
            filter(squeeze(mIIR_B(iCRx,iCTx,:)), squeeze(mIIR_A(iCRx,iCTx,:)), ...
                   mOut_Delayed(:,iCTx), mIIRReg(:,iCRx,iCTx));
    ...
    % 注意：如果 FIR 之前的 mIn (来自 fileReader) 才是原始输入，
    % 则应该存 mIn。这里假设 mIn 是正确的原始输入信号。
